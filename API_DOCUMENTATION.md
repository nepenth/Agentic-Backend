...
```javascript
const ws = new WebSocket('wss://ollama.example.invalid/ws/logs');
```
...
const wsUrl = window.location.protocol === 'https:'
  ? 'wss://ollama.example.invalid/ws/logs'
...
function TaskMonitor({ taskId }) {
  const wsUrl = `wss://ollama.example.invalid/ws/tasks/${taskId}`;
...
wss://ollama.example.invalid/ws/logs?task_id=456
